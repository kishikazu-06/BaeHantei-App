...
def load_model():
    ...
    # ここではローカルのyolov5リポジトリを指定し、pretrained=Trueで事前学習済みモデルを使用します。
    model = torch.hub.load(YOLOV5_PATH, 'yolov5s', source='local', pretrained=True)
    
    # モデルがロードされたことを確認するためのデバッグ出力
    print("Model loaded successfully using torch.hub.load (local source).")
